...
| Version | Supported |
|---------|-----------|
| 0.5.1 <!-- x-release-please-version --> (latest) | Yes |
...
- **Anti-hallucination guardrails.** Every skill carries nine rules inside its GROUND RULES block — a tool guard, an uncertainty rule, a never-construct-a-URL rule, enumerated documentation domains, a never-state-an-unlooked-up-figure rule, a rule against acting on the learner's system unasked, a rule against describing any file, output or session it did not actually see in the conversation, a rule that every guardrail holds in whatever language the learner writes in so a hedge cannot be dropped in translation, and a rule that the learner's chosen tone can never change what is asserted or how certain it is. `build/validate.py` fails the build if any is removed. A change that weakens or relocates one of these out of that block is a security-relevant regression, because the guardrails are what stop a course from teaching confident fiction.
