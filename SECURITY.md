...
- **Anti-hallucination guardrails.** Every skill carries six rules inside its GROUND RULES block — a tool guard, an uncertainty rule, a never-construct-a-URL rule, enumerated documentation domains, a never-state-an-unlooked-up-figure rule, and a rule that the learner's chosen tone can never change what is asserted or how certain it is. `build/validate.py` fails the build if any is removed. A change that weakens or relocates one of these out of that block is a security-relevant regression, because the guardrails are what stop a course from teaching confident fiction.
