...
- **Every skill opens at least one eval case under `evals/`**, and the validator fails if one does not. A change to a guardrail is a change to behaviour; run `claude plugin eval . --trust-plugin --ablation none --no-publish` before claiming it still holds. The suite checks behaviour, it does not prove it: a passing case means the reply satisfied its graders on that run.
- **Nine guardrails must live inside the GROUND RULES block of every skill** — the tool guard, the uncertainty rule, the never-construct-a-URL rule, enumerated doc domains, the never-state-an-unlooked-up-figure rule, the never-act-on-their-system rule, the never-invent-what-you-did-not-read-or-run rule, the learner's-language rule, and the rule that tone never changes what is true. They sit there rather than lower down so a truncated paste still carries them. `build/validate.py` fails the build if any is missing; the reason for each sits in the comment beside its gate in `build/validate.py`.
