...
- **Secrets never appear in code, chat, or examples.** API keys live in environment variables (e.g. `ANTHROPIC_API_KEY`), never hardcoded, never pasted into a chat, never committed. Every example uses a placeholder. (`/claude-docent:security` goes deep on this.)
...
2. **If you can read files**, look for `~/.claude-docent/progress-builder.md`, then `./.claude-docent/progress-builder.md`. If one exists, read it and treat it as the source of truth.
...
**Save it properly.** If you can write files, write the card to `~/.claude-docent/progress-builder.md` (create the directory if needed; fall back to `./.claude-docent/`), overwriting the previous version. Tell them the exact path the first time. Also show the card in the chat. **Never write a secret, key, or token into the card.**
...
`/claude-docent:foundations` (fundamentals), `/claude-docent:prompt-craft` (better prompts — directly useful for system prompts), `/claude-docent:reliability` (grounding and evals), `/claude-docent:security` (tool/injection/secret safety — essential for builders), `/claude-docent:mechanics` (models, context, cost).
