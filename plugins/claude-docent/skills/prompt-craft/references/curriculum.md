...
- **2.6 Delimiters & structure:** separating *instructions* from the *content* Claude should act on (headings, quotes, fenced blocks) so it never confuses the two — a habit that also quietly reduces prompt-injection risk (see `/claude-docent:security`). Exercise: restructure a messy prompt so instructions and source material are clearly separated.
- **2.7 Level 2 boss fight:** hit an exact spec — a given format + tone + length — in a single prompt, no back-and-forth. Pass = Level 3.
