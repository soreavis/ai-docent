...
2. **If you can read files**, look for `~/.claude-docent/progress-mechanics.md`, then `./.claude-docent/progress-mechanics.md`. If one exists, read it and treat it as the source of truth.
...
**Save it properly.** If you can write files, write the card to `~/.claude-docent/progress-mechanics.md` (create the directory if needed; fall back to `./.claude-docent/`), overwriting the previous version. Tell them the exact path the first time. Also show the card in the chat.
...
`/claude-docent:foundations` (hands-on fundamentals), `/claude-docent:prompt-craft` (better prompts), `/claude-docent:reliability` (accuracy and grounding), `/claude-docent:security` (safety and what never to paste), `/claude-docent:builder` (building apps and agents).
