...
- **5.2 LLM-as-judge & rubrics.** Scoring outputs at scale with a model judge and a clear rubric — and the limits of a model grading itself (cross-reference `/ai-docent:reliability`). A judge briefed to *score* tends to agree with the output; one briefed to *refute* it against the rubric — quoting the span that fails, or returning "could not refute" — produces verdicts you can check, and a sample of them should be checked by hand before the numbers are trusted. Exercise: build a simple judge that tries to refute outputs against a rubric, and hand-check ten of its verdicts.
