...
- **1.7 Memory, past chats & long-game habits:** how continuity works, when to start fresh vs. continue, naming chats, what Claude remembers. One habit for anything else they're learning: have it quiz them and withhold the answers, or explain the topic back and ask to be marked harshly. Exercise: design their personal "Claude workspace" — projects they'll actually keep — or set up one quiz-me session on something they're learning elsewhere.
...
- **2.2 Install & first contact:** install Claude Code (verify current steps in official docs first!), open it in a folder, ask questions about files, make a first tiny change and see it happen. The same move on a project they didn't write: ask what it does and which files matter for the part they care about, then open one file it named and check, because a tour is a claim. Exercise: have Claude Code introduce the machine to them — "what's in this folder, explain it like I'm new" — then check one thing it said against the files.
...
- **3.3 Data, documents & analysis:** giving Claude spreadsheets/CSVs, getting charts, summaries, and answers; sanity-checking the numbers. The same for the messy documents of a working week — meeting notes into decisions, commitments and open questions, with a fourth list for what fit none of them, because a summary drops things. Exercise: analyze real data from their life or studies (expenses, grades, stats from a hobby), or run one messy document from their week through that shape.
...
- **4.3 MCP, builder side:** how MCP servers work conceptually; configure/run an existing one for a tool they use; (if coding-inclined) sketch a minimal custom one with Claude Code's help. (For the full build, `/ai-docent:builder`.)
...
- **4.7 The API & building AI features:** calling Claude from code; build a tiny app *powered by* Claude rather than built with it. Exercise: a 30-line script or mini-app that uses the API for something fun. (Then `/ai-docent:builder` for the real track.)
